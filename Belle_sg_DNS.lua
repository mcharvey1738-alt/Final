...
local SaveManager     = loadstring(game:HttpGet("https://raw.githubusercontent.com/StyearX/Fluent-Modded/releases/download/Fluent/SaveManager"))()
local InterfaceManager= loadstring(game:HttpGet("https://raw.githubusercontent.com/StyearX/Fluent-Modded/releases/download/Fluent/InterfaceManager"))()
local MediaManager     = loadstring(game:HttpGet("https://raw.githubusercontent.com/StyearX/Fluent-Modded/releases/download/Fluent/MediaManager"))()
local FloatingButtonManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/StyearX/Fluent-Modded/releases/download/Fluent/FloatingButtonManager"))()
    ...
    task.spawn(function()
        ...
        for _,v in next,getgc(true) do
            if typeof(v)=="function" then
                local s = ""
                pcall(function() s = (debug and debug.info) and debug.info(v, "s") or "" end)
                if s == "" then n = n + 1 end
            end
        end
...
-- ── Floating Button (Purple + Belle.sg logo) ─────────────────────────────────
...
local function CreateButton(ButtonName, Name, Size1, Size2, ScriptLogic, CircleMode)
    ...
    stroke.Thickness = 3; stroke.Transparency = 0.8
    ...
    local function applyShape(circle)
        ...
        if circle then
            ...
            button.TextWrapped = true; button.TextScaled = true
            Instance.new("UICorner", frame).CornerRadius = UDim.new(1, 0)
        else
            frame.Size = originalSize
            button.TextWrapped = false; button.TextScaled = true
            Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 4)
        end
    end
    applyShape(isCircle)
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if inp == dragInput and dragging and not frame:GetAttribute("Locked") then
            local delta = inp.Position - dragStart
            frame.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)

    if ScriptLogic then button.Activated:Connect(function() ScriptLogic(button) end) end
    return frame, button, applyShape
end

-- Purple floating toggle button with Belle.sg logo
local _toggleGui = Instance.new("ScreenGui")
_toggleGui.Name = "BelleSGFloating"
_toggleGui.Parent = game:GetService("Players").LocalPlayer.PlayerGui
_toggleGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
_toggleGui.ResetOnSpawn = false

local _mainBtn = Instance.new("TextButton")
_mainBtn.Name = "FloatBtn"
_mainBtn.Parent = _toggleGui
_mainBtn.BackgroundColor3 = Color3.fromRGB(139, 92, 246)
_mainBtn.BackgroundTransparency = 0.15
_mainBtn.Position = UDim2.new(0.05, 0, 0.08, 0)
_mainBtn.Size = UDim2.new(0, 52, 0, 52)
_mainBtn.Text = ""
_mainBtn.Visible = true
Instance.new("UICorner", _mainBtn).CornerRadius = UDim.new(1, 0)
local _btnStroke = Instance.new("UIStroke", _mainBtn)
_btnStroke.Color = Color3.fromRGB(200, 160, 255)
_btnStroke.Thickness = 1.5

-- Belle.sg logo icon
local _btnIcon = Instance.new("ImageLabel", _mainBtn)
_btnIcon.Size = UDim2.new(0, 36, 0, 36)
_btnIcon.Position = UDim2.new(0.5, 0, 0.5, 0)
_btnIcon.AnchorPoint = Vector2.new(0.5, 0.5)
_btnIcon.BackgroundTransparency = 1
_btnIcon.Image = "rbxassetid://74730846535909"
_btnIcon.ScaleType = Enum.ScaleType.Fit
_btnIcon.ZIndex = 2
Instance.new("UICorner", _btnIcon).CornerRadius = UDim.new(0.2, 0)

-- Draggable
local _dragging, _dragInput, _dragStart, _startPos = false, nil, nil, nil
_mainBtn.InputBegan:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
        _dragging = true; _dragStart = inp.Position; _startPos = _mainBtn.Position
        inp.Changed:Connect(function()
            if inp.UserInputState == Enum.UserInputState.End then _dragging = false end
        end)
    end
end)
_mainBtn.InputChanged:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch then
        _dragInput = inp
    end
end)
UserInputService.InputChanged:Connect(function(inp)
    if inp == _dragInput and _dragging then
        ...
        _mainBtn.Position = UDim2.new(_startPos.X.Scale, _startPos.X.Offset + delta.X, _startPos.Y.Scale, _startPos.Y.Offset + delta.Y)
    end
end)

local _uiOpen = true
_mainBtn.MouseButton1Click:Connect(function()
    -- Only toggle if not dragging
    _uiOpen = not _uiOpen
    if _uiOpen then Window:Show() else Window:Hide() end
    -- Purple pulse effect
    local ts = game:GetService("TweenService")
    ts:Create(_mainBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(180, 130, 255)}):Play()
    task.wait(0.15)
    ts:Create(_mainBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(139, 92, 246)}):Play()
end)

MediaManager:SetFolder("BelleSgDNS/MediaCache")
...
task.delay(0.3, function() Window:SelectTab(1) end)
